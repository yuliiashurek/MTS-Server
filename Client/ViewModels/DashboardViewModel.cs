...
namespace Client.ViewModels.MenuControls
{
    public partial class DashboardViewModel : ObservableObject
    {
        ...
        [ObservableProperty]
        private ObservableCollection<MaterialCurrentStockDto> filteredMaterials = new();
        ...
        partial void OnMaterialsChanged(ObservableCollection<MaterialCurrentStockDto> value)
        {
            ApplyFilter();
        }
        ...
        private void ApplyFilter()
        {
            ...
            {
                FilteredMaterials = new ObservableCollection<MaterialCurrentStockDto>(Materials);
            }
            else
            {
                var lower = FilterText.ToLower();

                FilteredMaterials = new ObservableCollection<MaterialCurrentStockDto>(
                    Materials.Where(m => m.MaterialName?.ToLower().StartsWith(lower) == true));
                    //Materials.Where(m => m.MaterialName?.ToLower().Contains(lower) == true));
            }

            if (!FilteredMaterials.Contains(SelectedMaterial))
                SelectedMaterial = FilteredMaterials.FirstOrDefault();
        }
    }
}
